package com.boss.nettyquickstart.handler;
...
/**
 * @Author 黄杰峰
 * @Date 2020/7/19 0019 14:36
 * @Description 创建自定义助手类
 *              处理的请求是：客户端向服务端发起送数据，
 *              先把数据放在缓冲区，服务器端再从缓冲区读取，
 *              类似于[ 入栈, 入境 ]
 */

public class CustomerHandler extends SimpleChannelInboundHandler<HttpObject>{//Http请求，所以使用HttpObject

    private static Logger logger = LogManager.getLogger(LogManager.ROOT_LOGGER_NAME);
    
    @Override
    public void channelRegistered(ChannelHandlerContext ctx) throws Exception {
        logger.info("channel注册");
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        logger.info("channel活跃状态");
        super.channelActive(ctx);
    }

    @Override
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        logger.info("客户端与服务端断开连接之后");
        super.channelInactive(ctx);
    }

    @Override
    public void channelReadComplete(ChannelHandlerContext ctx) throws Exception {
        logger.info("channel读取数据完毕");
    ...
    protected void channelRead0(ChannelHandlerContext ctx, HttpObject msg) throws Exception {
        ...
        if(msg instanceof HttpRequest) {
            ...
            //客户端远程地址
            logger.info(currentChannel.remoteAddress());
            /**
             *
             * 未加判断类型，控制台打印的远端地址如下：
             *
